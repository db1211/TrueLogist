...
const EditPromo = () => {
    ...
    const handelUpdate = () => {
        settings.AllowCriticalEditsAdmin ?
            new Promise((resolve, reject) => {
                setLoding(true);
                setTimeout(() => {
                    if (!(data.promo_name && data.promo_code && data.promo_description && data.promo_discount_value && data.promo_discount_type && data.promo_discount_value && data.max_promo_discount_value && data.min_order && data.promo_usage_limit && (data.min_order >= data.max_promo_discount_value))) {
                        setLoding(false);
    ...
    const handelAdd = () => {
        new Promise((resolve, reject) => {
            setLoding(true);
            setTimeout(() => {
                if (!(data.promo_name && data.promo_code && data.promo_description && data.promo_discount_value && data.promo_discount_type && data.promo_discount_value && data.max_promo_discount_value && data.min_order && data.promo_usage_limit && (data.min_order >= data.max_promo_discount_value) && data.promo_validity)) {
                    setLoding(false);
                    setCommonAlert({ open: true, msg: t("no_details_error") });
                    reject();
                } else {
                    if(promoCodes.includes(data.promo_code.toUpperCase())){
                        setLoding(false);
                        setCommonAlert({ open: true, msg: t("code_already_avilable") });
                    }else{
                    data['promo_code'] = data.promo_code.toUpperCase();
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={6} xl={6} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                        <Button
                            ...
                            onClick={id ? handelUpdate : handelAdd}
                            variant="contained"
                        >
                            <Typography
                                style={{
